...
class RecommendationEngine:
    ...
    def validate_answer(self, insurance_type: str, question_id: str, answer: Any) -> tuple[bool, Optional[str]]:
        """
    ...
    def recommend_products(
        ...
        for product in products:
            score, reasons = self._score_product(product, user_requirements, insurance_type)
        ...
        if ai_provider and recommendations:
            recommendations = self._enhance_with_ai(recommendations, user_requirements, ai_provider)
    ...
    def _score_health_insurance(
        ...
        if has_conditions:
            if "Covered from day 1" in product["eligibility"]["pre_existing_conditions"]:
                score += 25
                reasons.append("Immediate coverage for pre-existing conditions")
            elif "Limited coverage" in product["eligibility"]["pre_existing_conditions"]:
                score += 10
                reasons.append("Some coverage for pre-existing conditions after waiting period")
        else:
            score += 10
        ...
        if budget:
            ...
            min_premium = int(premium_range.split(" - ")[0].replace("$", "").replace(",", ""))
            max_premium = int(premium_range.split(" - ")[1].replace("$", "").replace(",", ""))
        ...
        elif preference == "international_coverage" and "International coverage" in product.get("features", []):
            score += 15
    ...
    def _score_life_insurance(
        ...
        if income:
            ...
            if "Minimum" in income_req:
                min_income = int(income_req.split("$")[1].replace(",", "").split("/")[0])
        ...
        if has_coverage and product["type"] == "Whole Life":
            score += 10
            reasons.append("Good supplement to existing coverage with investment component")
    ...
    def _score_auto_insurance(
        ...
        if violations == "none":
            ...
            if "Clean record preferred" in driving_history:
                score += 10
        elif violations in ["1_violation", "1_accident"] and "Acceptable record" in driving_history:
            score += 15
        ...
        if vehicle_age == "very_old_10+_years" or vehicle_value < 5000:
            if "Liability Only" in product["type"]:
                score += 25
                reasons.append("Liability-only coverage recommended for older/low-value vehicles")
            else:
                score -= 10
                reasons.append("Full coverage may not be cost-effective for this vehicle")
        elif vehicle_age == "new_0-3_years" or vehicle_value > 25000:
            if "Full Coverage" in product["type"]:
                score += 25
                reasons.append("Full coverage protects your valuable vehicle investment")
    ...
    def _score_home_insurance(
        ...
        if prop_value:
            ...
            if "+" in value_range:
                ...
                min_val = int(value_range.replace("$", "").replace(",", "").replace("+", ""))
                ...
                min_val = int(value_range.split(" - ")[0].replace("$", "").replace(",", ""))
                max_val_str = value_range.split(" - ")[1].replace("$", "").replace(",", "").replace("+", "")
                ...
                if "+" in value_range.split(" - ")[1]:
                    ...
                    if min_val <= prop_value <= max_val:
                        ...
                        score -= 10
                        reasons.append("Property value exceeds this plan's coverage limit")
        ...
        if contents > 100000 and "high-value personal property" in str(product.get("features", [])).lower():
            score += 20
