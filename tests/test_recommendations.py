...
class TestRecommendationEngine:
    ...
    def test_validate_answer_number_valid(self):
        ...
        is_valid, error = self.engine.validate_answer("health_insurance", "age", "30")
        assert is_valid is True
        assert error is None

    def test_validate_answer_number_below_min(self):
        """Test validation of number below minimum"""
        is_valid, error = self.engine.validate_answer("health_insurance", "age", "10")
        assert is_valid is False
        assert "at least" in error.lower()

    def test_validate_answer_number_above_max(self):
        """Test validation of number above maximum"""
        is_valid, error = self.engine.validate_answer("health_insurance", "age", "150")
        assert is_valid is False
    ...
    def test_recommend_health_insurance_pre_existing(self):
        ...
        assert any(
            "pre-existing" in reason.lower() for reason in top_recommendation["reasons"]
        )
    ...
    def test_recommend_life_insurance_young_single(self):
        ...
        recommendations = self.engine.recommend_products("life_insurance", requirements)
    ...
    def test_recommend_auto_insurance_new_car(self):
        ...
        recommendations = self.engine.recommend_products("auto_insurance", requirements)

        assert len(recommendations) > 0
        # Premium Auto Protection recommended for new expensive car
        top_recommendation = recommendations[0]
        assert (
            "Premium" in top_recommendation["product"]["name"]
            or "Full Coverage" in top_recommendation["product"]["type"]
        )
    ...
    def test_recommend_auto_insurance_multi_vehicle(self):
        ...
        assert any(
            "Family" in name for name in top_two_names
    ...
    def test_recommend_home_insurance_basic(self):
        ...
        recommendations = self.engine.recommend_products("home_insurance", requirements)
    ...
    def test_recommend_home_insurance_high_risk(self):
        ...
        assert (
            "Premium" in top_recommendation["product"]["name"]
            or "Luxury" in top_recommendation["product"]["name"]
        )
    ...
    def test_all_insurance_types_have_questionnaires(self):
        ...
        for insurance_type in PRODUCT_CATALOG.keys():
            ...
            ), f"Missing questionnaire for {insurance_type}"
            assert len(questionnaire["questions"]) > 0
    ...
    def test_all_insurance_types_have_products(self):
        ...
        for insurance_type, data in PRODUCT_CATALOG.items():
            ...
            assert (
                len(data["products"]) >= 3
            ), f"{insurance_type} should have at least 3 product variants"
        ...
        for insurance_type, data in PRODUCT_CATALOG.items():
            for product in data["products"]:
                for field in required_fields:
                    assert (
                        field in product
                    ), f"Product {product.get('name', 'unknown')} missing field: {field}"

                # Check eligibility - only auto, health, and life have age requirements
                if insurance_type in [
                    "health_insurance",
                    "life_insurance",
                    "auto_insurance",
                ]:
                    assert (
                        "min_age" in product["eligibility"]
                    ), f"{product['name']} missing min_age"
                    assert (
                        "max_age" in product["eligibility"]
                    ), f"{product['name']} missing max_age"
    ...
    def test_recommendation_reasons_not_empty(self):
        ...
        for insurance_type, requirements in test_cases:
            ...
            for rec in recommendations:
                assert (
                    len(rec["reasons"]) > 0
                ), f"No reasons for {rec['product']['name']}"
                assert (
                    rec["match_percentage"] > 0
                ), f"Match percentage is 0 for {rec['product']['name']}"
